...
- (void)viewDidLoad {
    ...
    self.searchDisplayController.searchBar.placeholder = @"搜索位置";
    _region=MKCoordinateRegionMakeWithDistance(self.mapView.userLocation.coordinate,800,800);
    [self.mapView setRegion:_region];
    
    //   加载topBar
    self.view.backgroundColor=mRGBColor(236, 235, 235);
    UIImageView*topBarImag=[[UIImageView alloc] initWithFrame:CGRectMake(0,548-44, self.view.bounds.size.width, 44)];
    ...
    [closeMenuBtn setFrame:CGRectMake(10, 548-40, 50, 44)];
...
- (IBAction)recenterMapToUserLocation:(id)sender {
    MKCoordinateRegion regionV;
    MKCoordinateSpan spanV;
    
    spanV.latitudeDelta = 0.02;
    spanV.longitudeDelta = 0.02;
    
    regionV.span = spanV;
    regionV.center = self.mapView.userLocation.coordinate;
    
    [self.mapView setRegion:regionV animated:YES];
}
